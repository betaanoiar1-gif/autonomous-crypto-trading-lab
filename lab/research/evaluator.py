...
def _param_grid(family: str, base: dict) -> list[dict]:
    ...
    if family in {"momentum", "breakout", "trend_pullback"}:
        ...
        if family == "trend_pullback":
            t = float(base.get("pullback_threshold", 0.01))
            thresholds = sorted({round(max(0.001, min(0.10, t * f)), 4) for f in (0.75, 1.0, 1.25)})
            return [{"lookback": n, "pullback_threshold": x} for n, x in itertools.product(vals, thresholds)][:15]
    ...
    if family == "moving_average_cross":
        fast = int(base.get("fast", 10)); slow = int(base.get("slow", 40))
        fs = sorted({max(2, min(100, int(fast * f))) for f in (0.75, 1.0, 1.25)})
        ss = sorted({max(3, min(300, int(slow * f))) for f in (0.75, 1.0, 1.25, 1.5)})
        return [{"fast": f, "slow": s} for f, s in itertools.product(fs, ss) if s > f][:12]
    if family == "rsi_reversion":
        n = int(base.get("rsi_length", 14)); lo = float(base.get("rsi_low", 30)); hi = float(base.get("rsi_high", 70))
        ns = sorted({max(2, min(50, int(n * f))) for f in (0.75, 1.0, 1.25)})
        los = sorted({max(5, min(45, lo + d)) for d in (-5, 0, 5)})
        his = sorted({max(55, min(95, hi + d)) for d in (-5, 0, 5)})
        return [{"rsi_length": n0, "rsi_low": l, "rsi_high": h} for n0, l, h in itertools.product(ns, los, his) if l < 50 < h][:15]
    if family == "atr_breakout":
        n = int(base.get("atr_length", 14)); m = float(base.get("atr_mult", 1.5))
        ns = sorted({max(2, min(50, int(n * f))) for f in (0.75, 1.0, 1.25)})
        ms = sorted({round(max(0.25, min(5.0, m * f)), 3) for f in (0.75, 1.0, 1.25)})
        return [{"atr_length": n0, "atr_mult": m0} for n0, m0 in itertools.product(ns, ms)]
    if family == "channel_reversion":
        n = int(base.get("channel_length", 40))
        vals = sorted({max(5, min(200, int(n * f))) for f in (0.75, 1.0, 1.25, 1.5)})
...
def _training_objective(metrics: dict) -> float:
    ...
    ret = float(metrics.get("total_return", 0.0)); pf = min(3.0, float(metrics.get("profit_factor", 0.0)))
    dd = abs(min(0.0, float(metrics.get("max_drawdown", 0.0)))); sh = float(metrics.get("sharpe", 0.0))
...
def _select_params(train, family, base_params, directions, capital, fee_bps, slippage_bps):
    ...
    for candidate in _param_grid(family, base_params):
        m = _metrics(_run(train, family, candidate, directions, capital, fee_bps, slippage_bps), _run(train, family, candidate, directions, capital, fee_bps, slippage_bps).returns)
        score = _training_objective(m); leaderboard.append({"parameters": candidate, "score": float(score), "metrics": m})
        if best is None or score > best[0]: best = (score, candidate, m)
...
def _walk_forward(df, family, base_params, directions, capital, fee_bps, slippage_bps, windows=4):
    n = len(df)
    if n < 240:
        return {"windows": [], "positive_windows": 0, "median_return": 0.0, "positive_ratio": 0.0, "median_sharpe": 0.0, "min_trade_count": 0, "passed": False, "mode": "tuned"}
    ...
    while len(rows) < windows and start + train_size + test_size <= n:
        ...
        m = _metrics(_run(test, family, selected, directions, capital, fee_bps, slippage_bps), _run(test, family, selected, directions, capital, fee_bps, slippage_bps).returns)
        rows.append({"fold": len(rows) + 1, "train_start": str(train.index[0]), "train_end": str(train.index[-1]), "test_start": str(test.index[0]), "test_end": str(test.index[-1]), "selected_parameters": selected, "tuning": tuning, "total_return": float(m["total_return"]), "max_drawdown": float(m["max_drawdown"]), "profit_factor": float(m["profit_factor"]), "sharpe": float(m["sharpe"]), "trade_count": int(m["trade_count"])})
        start += test_size
    if not rows:
        return {"windows": [], "positive_windows": 0, "median_return": 0.0, "positive_ratio": 0.0, "median_sharpe": 0.0, "min_trade_count": 0, "passed": False, "mode": "tuned"}
...
def evaluate(df, family, params, directions, initial_capital, fee_bps, slippage_bps, holdout_ratio=0.30):
    ...
    selected, tuning = _select_params(train, family, params, directions, initial_capital, fee_bps, slippage_bps)
    a = _run(train, family, selected, directions, initial_capital, fee_bps, slippage_bps); b = _run(test, family, selected, directions, initial_capital, fee_bps, slippage_bps)
    variants = []
    for key, base in selected.items():
        try:
            delta = max(1, abs(base) * 0.20) if isinstance(base, int) else max(0.01, abs(base) * 0.20)
            for value in (base - delta, base + delta):
                ...
                variants.append(_run(train, family, p, directions, initial_capital, fee_bps, slippage_bps).metrics["total_return"])
        except (TypeError, ValueError): pass
    stability = bool(not variants or min(variants) > a.metrics["total_return"] - 0.25)
    stressed = _run(test, family, selected, directions, initial_capital, fee_bps * 2, slippage_bps * 2)
    walk = _walk_forward(df, family, selected, directions, initial_capital, fee_bps, slippage_bps)
