...
def _feature_series(features: pd.DataFrame, name: str) -> pd.Series:
    if name == "mom":
        return features["mom_24"]
    if name == "mean_distance":
        return features["mean_distance_72"]
    if name == "breakout_pressure":
        return features["breakout_pressure_48"]
    if name == "vol_scaled_mom":
        return features["vol_scaled_mom"]
    if name == "vol_compression":
        return features["vol_compression"]
    if name == "volume_pressure":
        return features["volume_pressure"]
    if name == "trend_strength":
        return features["trend_strength"]
    raise KeyError(name)


def _purged_blocks(frame: pd.DataFrame, horizon: int, blocks: int = 5, purge: int = 72) -> list[tuple[pd.DataFrame, pd.DataFrame]]:
    n = len(frame)
    if n < 5000:
        return []
    usable = n - purge
    ...
    for k in range(blocks):
        train_end = (k + 1) * width
        test_start = min(train_end + purge, n)
        test_end = min(test_start + width, n)
...
def _stats(x: pd.Series, y: pd.Series) -> tuple[float, float, float, float, float, float]:
    ...
    values = [ic, rank_ic, sign_accuracy, spread, top_ret, bottom_ret, turnover_proxy]
...
def _aggregate(rows: list[EdgeStats]) -> list[dict]:
    grouped: dict[tuple[str, int], list[EdgeStats]] = {}
    for row in rows:
        grouped.setdefault((row.feature, row.horizon), []).append(row)
    out: list[dict] = []
    for (feature, horizon), items in grouped.items():
        ics = [x.ic for x in items]
        rics = [x.rank_ic for x in items]
        spreads = [x.top_bottom_spread for x in items]
        accs = [x.sign_accuracy for x in items]
        out.append({
            "feature": feature,
            "horizon": horizon,
            "markets": len(items),
            "median_ic": float(np.median(ics)),
            "median_abs_ic": float(np.median(np.abs(ics))),
            "median_rank_ic": float(np.median(rics)),
            "median_spread": float(np.median(spreads)),
            "positive_spread_markets": int(sum(x > 0 for x in spreads)),
            "median_sign_accuracy": float(np.median(accs)),
        })
    return sorted(
        out,
        key=lambda x: (
            x["positive_spread_markets"],
            abs(x["median_spread"]),
            abs(x["median_rank_ic"]),
        ),
        reverse=True,
    )


def _permutation_test(df: pd.DataFrame, feature: str, horizon: int, trials: int = 50, seed: int = 20260829) -> dict:
    ...
    return {
        "trials": trials,
        "false_positive_rate": float(exceed / trials),
        "observed": observed,
    }
...
def run(minutes: float = 60.0, seed: int = 20260829) -> dict:
    ...
    for row in ranking[:12]:
        print(
            f"edge {row['feature']} h={row['horizon']} "
            f"IC={row['median_ic']:.4f} RankIC={row['median_rank_ic']:.4f} "
            f"spread={row['median_spread']:.4%} "
            f"spread+={row['positive_spread_markets']}/{row['markets']} "
            f"acc={row['median_sign_accuracy']:.2%}",
            flush=True,
        )
    ...
    for idx, row in enumerate(candidates, 1):
        if time.monotonic() >= deadline:
            break
        market = "BTC/USDT"
        p = _permutation_test(dev[market], row["feature"], row["horizon"], 50, seed + idx)
        p.update({"feature": row["feature"], "horizon": row["horizon"]})
        permutation.append(p)
        print(
            f"perm {idx}/{len(candidates)} | {row['feature']} h={row['horizon']} "
            f"observed={p['observed']:.4f} exceed={p['false_positive_rate']:.2%}",
            flush=True,
        )
    ...
    for idx, row in enumerate(candidates, 1):
        ...
        validation.append({
            **row,
            "rank": idx,
            "median_ic": float(np.median([v["ic"] for v in vals])),
            "median_rank_ic": float(np.median([v["rank_ic"] for v in vals])),
            "median_spread": float(np.median([v["spread"] for v in vals])),
            "positive_spread_markets": int(sum(v["spread"] > 0 for v in vals)),
            "median_sign_accuracy": float(np.median([v["accuracy"] for v in vals])),
            "market_details": vals,
        })
        print(
            f"validation {idx}/{len(candidates)} | {row['feature']} h={row['horizon']} "
            f"IC={validation[-1]['median_ic']:.4f} "
            f"spread={validation[-1]['median_spread']:.4%} "
            f"positive={validation[-1]['positive_spread_markets']}/12",
            flush=True,
        )
    ...
    for idx, row in enumerate(frozen, 1):
        ...
        lockbox.append(result)
        print(
            f"lockbox {idx}/3 | {row['feature']} h={row['horizon']} "
            f"IC={result['median_ic']:.4f} spread={result['median_spread']:.4%} "
            f"positive={result['positive_spread_markets']}/12",
            flush=True,
        )

    eligible = [
        x for x in lockbox
        if x["positive_spread_markets"] >= 7
        and x["median_spread"] > 0
        and x["median_sign_accuracy"] > 0.50
    ]
    ...
    payload = {
        ...
        "protocol": {
            ...
            "future_target": "close.pct_change(horizon).shift(-horizon)",
            "purged_validation": True,
            "spot_only": True,
            "ai": False,
            "futures": False,
        },
