...
def _eval_one(idx, h, spot, futures, funding, futures_error, settings, run_id):
    ...
    try:
        ...
        ev = evaluate_fixed(
            data[(symbol, timeframe)], h.executable_family, h.executable_parameters,
        ...
        if status in {"VALIDATION_CANDIDATE", "VALIDATED"}:
            params = dict(h.executable_parameters)
...
def run(max_hypotheses: int = 12, agent=None) -> dict:
    ...
    print(f"Data source: binance | timeframes={SUPPORTED_TIMEFRAMES} | symbols={SYMBOLS}", flush=True)
    print(f"Spot observations: {sum(snapshot['observations_spot'].values())} total rows across pairs/factors", flush=True)
    if isinstance(futures, dict) and not futures_error:
        print(f"Futures data: connected | historical funding loaded | events={sum(len(v) for v in funding.values())} | cache=active", flush=True)
    elif futures_error:
        print(f"Futures data: unavailable -> {futures_error}", flush=True)

    target = min(max_hypotheses, len(DIVERSITY_SLOTS))
    print(f"Research slots requested: {target}", flush=True)
    print("ULTRA-FAST MODE: local generation + frozen-parameter WF", flush=True)
    hypotheses = generate(prior, target)
    for i, h in enumerate(hypotheses, 1):
        print(f"  Slot {i}: {h.executable_family} | {h.market_types[0].value} | {h.symbols[0]} | {h.timeframes[0]} | {_direction_value(h.directions[0])}", flush=True)
    print(f"Hypotheses generated: {len(hypotheses)}", flush=True)
    print(f"Parallel evaluation workers: {min(MAX_WORKERS, max(1, len(hypotheses)))}", flush=True)

    records = [None] * len(hypotheses)
    with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="acl-ultra") as pool:
        pending = {
            pool.submit(_eval_one, i, h, spot, futures, funding, futures_error, settings, run_id): i
            for i, h in enumerate(hypotheses)
        }
        for fut in as_completed(pending):
            records[pending[fut]] = fut.result()

    for i, (h, r) in enumerate(zip(hypotheses, records), 1):
        print(f"[{i}/{len(records)}] {h.title} | {h.symbols[0]} | {h.timeframes[0]} -> {r['status']}", flush=True)
    ...
    leaderboard = [
        {
            ...
            "symbol": r["symbol"], "timeframe": r["timeframe"], "market_type": r["market_type"],
    ...
    for r in leaderboard:
        print(
            f"  #{r['rank']} | {r['status']} | score={float(r['score']):.2f} | {r['title']} | "
            f"{r['symbol']} | {r['timeframe']} | {r['market_type']} | WF={r['walk_forward_passed']} | CONF={r['confirmation_passed']}",
            flush=True,
        )
    ...
    manifest = {
        ...
        "evaluation_workers": MAX_WORKERS,
        "ultra_fast": True,
        "frozen_parameter_evaluation": True,
    }
