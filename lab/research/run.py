...
DIVERSITY_SLOTS = [
    ...
    {"preferred_family": "breakout", "preferred_market": "futures", "preferred_timeframe": "4h", "preferred_symbol": "BTC/USDT", "preferred_direction": "both"},
    {"preferred_family": "moving_average_cross", "preferred_market": "futures", "preferred_timeframe": "1h", "preferred_symbol": "ETH/USDT", "preferred_direction": "short"},
...
def _safe_one_hypothesis(agent, snapshot, prior_failures, prior_hypotheses, slot):
    ...
    try:
        ...
        retry = ("Return exactly ONE hypothesis block. No JSON, markdown, analysis, or commentary. "
...
def _load_failures(path):
    ...
    for line in path.read_text(encoding="utf-8").splitlines()[-100:]:
        try:
            rec = json.loads(line)
            if rec.get("status") == "REJECTED": out.append(rec)
...
def _status(evaluation):
    if evaluation.passed: return "VALIDATED"
    oos, robust = evaluation.out_of_sample, evaluation.robustness
    if (float(oos.get("total_return", 0)) > 0 and float(oos.get("profit_factor", 0)) > 1 and
        float(oos.get("max_drawdown", 0)) >= -0.50 and float(robust.get("stressed_total_return", 0)) > 0):
        return "VALIDATION_CANDIDATE"
    return "REJECTED"


def _print_diagnostics(record):
    oos, robust = record.get("out_of_sample", {}), record.get("robustness", {})
...
def run(max_hypotheses: int = 8, agent=None) -> dict:
    ...
    snapshot = {"exchange": exchange_id, "available_timeframes": SUPPORTED_TIMEFRAMES, "symbols": SYMBOLS,
    ...
    for idx, h in enumerate(hypotheses):
        ...
        try:
            evaluation = evaluate(market[(symbol, timeframe)], h.executable_family, h.executable_parameters, directions,
                                  settings.capital.initial_usd, settings.execution.commission_bps, settings.execution.slippage_bps,
                                  settings.validation.holdout_ratio)
            status = _status(evaluation)
            record = {"run_id": run_id, "index": idx, "symbol": symbol, "timeframe": timeframe, "market_type": h.market_types[0].value,
            ...
            if status == "VALIDATED" and settings.output.generate_pine:
                ...
                p = out / f"validated_candidate_{idx+1}.pine"
                p.write_text(pine, encoding="utf-8")
                record["pine_path"] = str(p)
        except Exception as exc:
            record = {"run_id": run_id, "index": idx, "symbol": symbol, "timeframe": timeframe, "market_type": h.market_types[0].value,
