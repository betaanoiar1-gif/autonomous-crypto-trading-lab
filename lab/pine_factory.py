...
def build_pine(name: str, family: str, params: dict, allow_short: bool = True, commission_bps: float = 10.0,
               slippage_bps: float = 5.0, initial_capital: float = 500.0) -> str:
    family = family.lower().strip(); short = str(bool(allow_short)).lower()
    commission_pct = max(0.0, float(commission_bps)) / 100.0; slippage_ticks = max(0, int(round(float(slippage_bps))))
    ...
    if family == "momentum":
        ...
        n = int(max(2, min(200, _num(params, "lookback", 40)))); ze = _num(params, "z_entry", 1.5); zx = _num(params, "z_exit", 0.25)
        ...
        fast=int(max(2,min(100,_num(params,"fast",10)))); slow=int(max(fast+1,min(300,_num(params,"slow",40))))
        ...
        n=int(max(2,min(50,_num(params,"rsi_length",14)))); lo=_num(params,"rsi_low",30); hi=_num(params,"rsi_high",70)
        ...
        n=int(max(2,min(50,_num(params,"atr_length",14)))); mult=_num(params,"atr_mult",1.5)
        ...
        n=int(max(5,min(200,_num(params,"lookback",40)))); t=_num(params,"pullback_threshold",0.01)
    ...
    else: raise ValueError(f"Unsupported Pine family: {family}")
